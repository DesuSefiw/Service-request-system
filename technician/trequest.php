...
<div class="col-sm-4 mb-5">
  ...
  <?php 
 $sql = "SELECT request_id, request_info, request_desc,staff, assign_date ,assign_tech FROM technicians_res where staff='".$tecstaff."'AND assign_tech='".$tname."' ";
 ...
 if($result->num_rows >0){
   ...
   echo '<p class="card-text">description:' . $row['request_desc'] . '</p>';
      echo '<p class="card-text"> Department:' . $row['staff'] . '</p>';
   echo '<p class="card-text">Assign Date: ' . $row['assign_date'] . '</p>';
      echo '<p class="card-text">Technician Name: ' . $row['assign_tech'] . '</p>';
  ...
  <p>Aww yeah, you successfully Accept and Assign  all Requests.</p>
  <hr>
...
if(isset($_REQUEST['close'])){
  $sql = "DELETE FROM technicians_res WHERE request_id = {$_REQUEST['id']}";
