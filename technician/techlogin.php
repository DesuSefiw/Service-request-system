...
if(!isset($_SESSION['is_techlogin'])){
  if(isset($_REQUEST['tecEmail'])){
    $tecEmail = mysqli_real_escape_string($conn,trim($_REQUEST['tecEmail']));
    $tPassword = mysqli_real_escape_string($conn,trim($_REQUEST['tPassword']));
	 $tecstaff = mysqli_real_escape_string($conn,trim($_REQUEST['tecstaff']));
	 $tname = mysqli_real_escape_string($conn,trim($_REQUEST['tname']));

    $sql = "SELECT name ,email, password,staff FROM technicianlogin WHERE  name='".$tname."'AND email='".$tecEmail."' AND password='".$tPassword."'AND staff='".$tecstaff."'limit 1";
    $result = $conn->query($sql);
    if($result->num_rows == 1){
      $_SESSION['is_techlogin'] = true;
      $_SESSION['tecEmail'] = $tecEmail;
	  $_SESSION['tecstaff'] = $tecstaff;
...
<body>
  ...
  <p class="text-center" style="font-size: 20px;"> <i class="fas fa-user-secret text-danger"></i> <span>Techinician
      </span>
  </p>
    ...
    <div class="row justify-content-center custom-margin">
      <div class="col-sm-6 col-md-4">
		...
		<div class="form-group">
            <i class=""></i><label for="name" class="pl-2 font-weight-bold"></label>Name<input type="text"
              class="form-control" placeholder="name" name="tname">
          </div>
          <div class="form-group">
            <i class="fas fa-user"></i><label for="email" class="pl-2 font-weight-bold">Email</label><input type="email"
              class="form-control" placeholder="Email" name="tecEmail">
            <!--Add text-white below if want text color white-->
            <small class="form-text">We'll never share your email with anyone else.</small>
          </div>
		  <div class="form-group">
            <i class="fas fa-user"></i><label for="tecstaff" class="pl-2 font-weight-bold">Staff</label>
           
         
      <select class="form-control" placeholder="Departments" name="tecstaff" size="1">
