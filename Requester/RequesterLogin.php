...
if(!isset($_SESSION['is_login'])){
  if(isset($_REQUEST['rEmail'])){
	...
	$rstaff = mysqli_real_escape_string($conn,trim($_REQUEST['rstaff']));
    $rPassword = mysqli_real_escape_string($conn,trim($_REQUEST['rPassword']));
    $sql = "SELECT r_email, r_password FROM requesterlogin_tb WHERE r_email='".$rEmail."' AND r_password='".$rPassword."'AND staff='".$rstaff."' limit 1";
    $result = $conn->query($sql);
    if($result->num_rows == 1){
      
      $_SESSION['is_login'] = true;
      $_SESSION['rEmail'] = $rEmail;
	   $_SESSION['rstaff'] = $rstaff;
...
<body>
  <div class="mb-3 text-center mt-5" style="font-size: 30px;">
    ...
    <span>Online Service requeste Managment System</span>
  </div>
  ...
  <div class="container-fluid mb-5">
    <div class="row justify-content-center custom-margin">
      <div class="col-sm-6 col-md-4">
        <form action="" class="shadow-lg p-4" method="POST">
		  ...
		  <div class="form-group">
            <i class="fas fa-user"></i><label for="staff" class="pl-2 font-weight-bold">Staff</label> 
			<select class="form-control" placeholder="staff" name="rstaff" size="1">
    ...
    <option> Institutional Transformaton </option>
        ...
        <div class="text-center"><a style=background-color:black;  class="btn btn-info mt-3 shadow-sm font-weight-bold" href="../index.php">Back
            to Home</a></div>
      </div>
    </div>
  </div>
